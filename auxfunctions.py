    ...
    with open(output_file, 'w', encoding='utf-8') as file:
        for word in model.wv.index_to_key:
            try:
                ...
                similar_words_str = ', '.join([f"{w} ({s:.2f})" for w, s in most_similar])
...
def generate_most_similar_words_file(embeddings, vocab, output_file, top_k=10):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        for word in vocab.reverse_vocab.values():
            ...
            similar_words_with_scores = [f"{w} ({sim:.4f})" for w, sim in zip(similar_words, top_k_values)]
...
def visualize_embeddings_gensim(model, min_frequency=10, method="pca"):
    ...
    words = [
        ...
        if model.wv.get_vecattr(word, "count") >= min_frequency and word.lower() not in stop_words
    ]
...
def visualize_embeddings(word_embeddings, words, method="pca", perplexity=30, random_state=42):
    ...
    # Apply t-SNE transformation
        ...
        tsne = TSNE(n_components=2, perplexity=perplexity, random_state=random_state,metric="cosine")
