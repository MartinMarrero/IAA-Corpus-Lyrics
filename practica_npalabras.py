...
def compute_ngram_probabilities(ngrams, vocab, n):
    ...
    """
    cfd = defaultdict(lambda: defaultdict(int))  # Distribución condicional de frecuencias
...
def sentence_logprobability(sentence, cpd, n, vocab):
    ...
    for i in range(len(processed_sentence) - n + 1):
        ...
        if context in cpd:
            ...
            prob = cpd[context].get(token, 1 / len(vocab))  # Probabilidad uniforme para <UNK>
        else:
...
def generate_sentence_top_k(cpd, vocab, n, max_length=20, special_tokens_to_remove={"<s>", "</s>"}, k=20):
    """
    ...
    while len(sentence) < max_length:
        if context in cpd:
            ...
            top_k_words = sorted(probabilities.items(), key=lambda x: x[1], reverse=True)[:k]
    ...
    # Remover tokens especiales del resultado final
        tokens_return=[token for token in sentence if token not in special_tokens_to_remove]
        processed_words = []
...
if __name__ == "__main__":
    ...
    corpus = [['Hola', 'mundo', 'EOL', 'Mi', 'casa'], ['El', 'perro', 'EOL', 'El', 'gato']]
